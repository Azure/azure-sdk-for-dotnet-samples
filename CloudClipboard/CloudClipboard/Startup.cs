        ...
        private RequestDelegate EnsureUserIdAndContainer(RequestDelegate next)
        {
            return async ctx =>
            {
                ...
                try
                {
                    ...
                    await serviceClient.GetBlobContainerClient(userId).CreateAsync();
                }
                catch (StorageRequestFailedException ex) when (ex.ErrorCode == BlobErrorCode.ContainerAlreadyExists)
                {
                    // Ignore errors if the container already exists
                }
